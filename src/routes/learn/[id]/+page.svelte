<script lang="ts">
	...
	onMount(() => {
		document.cookie = `savedLesson=${page.params.id || 1}; Secure; Max-Age=31536000`
	})

	let { data } = $props();
	const Body = $derived(data.body);

	let objectiveCompleted = $state(false);
	$effect(() => {
		objectiveCompleted = window.localStorage.getItem(`lesson-${data.id}`) === '1';
	});
	...
	let code = $derived(data.content?.code ?? '');
	let output = $state('');
	let input = $state('');
	...
	function check() {
		objectiveCompleted = data.content?.expectedOutput.includes(output);
		localStorage.setItem(`lesson-${data.id}`, objectiveCompleted === true ? '1' : '0');
	}
</script>

<svelte:head>
	<title>learn - lesson {data.id} | aglang</title>
</svelte:head>
...
{#key data.id}
	<div id="wrapper">
		<div id="content-wrapper">
			...
			{#if data.content.objective && data.content.canContinue}
				<div id="objective-div">
					{#if data.content.objective}
						<div id="checkmark" class:completed={objectiveCompleted}></div>
						<span class:completed={objectiveCompleted}>{data.content.objective}</span>
					{/if}
					{#if data.content.canContinue}
						<a
							href="/learn/{data.next}"
							id="skip"
							class:disabled={objectiveCompleted || !data.content?.canContinue}
							onclick={() => {
								output = '';
							}}>skip</a
						>
		...
		<div id="editor-wrapper">
			<div id="top">
				...
				<div id="actions">
					...
					{#if data.content.expectedOutput}
						<span>Expected output:</span>
						<pre><code>{data.content.expectedOutput[0]}</code></pre>
						<button
							class="action-btn"
							onclick={() => {
								check();
...
<style lang="scss">
	...
	@keyframes checkmark {
		...
		100% {
			opacity: 100%;
			transform: translateY(15%) translateX(5%) rotate(45deg);
		}
	}
				...
				#checkmark {
					...
					&.completed {
						...
						&::after {
							...
							animation: checkmark 200ms forwards ease-out;
						}
					}
				}

				span.completed {
					animation: objective-completed 1.5s 200ms ease-in-out;
				}
